...
async function getLatestComments(): Promise<any> {
	...
	const latestReleaseResponse = (await (await fetch(
...
function parseApi(raw: RawComments): void {
	...
	const sourceCode = `// THIS FILE IS AUTOMATICALLY GENERATED WITH THE COMMAND "npm run build:obs-types"
// DO NOT EDIT
...
function unflattenAndResolveTypes(inputItems: RawType[]): Tree {
	...
	items.forEach(item => {
		...
		const parts = item.name.split('.').map(str => deburr(str.trim()));
